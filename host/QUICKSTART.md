...
2. **(선택) 첫 프로젝트 폴더 등록**
3. **(선택, claude 설치 시) claude 인증 토큰**
4. **aglink-\* 플러그인** — 형제 폴더에 없는 것마다 clone+빌드할지 하나씩 물어봄(Windows 전용).
   `aglink-web`을 새로 설치했다면 Chrome을 열어 `chrome://extensions`까지 띄워주는데,
   압축해제된 확장은 크롬 정책상 완전 무클릭 설치가 안 되어 "개발자 모드 켜기 → 압축해제된
   확장 프로그램을 로드합니다 → 폴더 선택"까지는 직접 클릭해야 합니다. 이 단계가 끝나면
   **설치된 aglink-chat/aglink-web은 자동으로 활성화됩니다** — 형제 폴더에 있는데 꺼진
...
aglink-\* 설정 등)은 그대로 둔 채 텔레그램 정보만 채워 넣습니다. 재시작하면 텔레그램에서도
바로 대화할 수 있습니다.
...
해줍니다. **Windows 전용**. 설정 마법사가 `aglink-screen`이 설치된 걸 보면 켤지 물어보므로
(민감한 권한이라 기본은 끔) 보통은 마법사에서 끝나지만, 나중에 수동으로 켜려면
`config.yaml`에 추가:
