<script>
  ...
  function statusBadge(status) {
    if (status === "pending") return "bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300";
    if (status === "paused") return "bg-amber-100 dark:bg-amber-900/40 text-amber-800 dark:text-amber-300";
...
<div class="flex h-full min-h-0 flex-col">
  <div class="flex h-11 shrink-0 items-center gap-2 border-b border-slate-200 dark:border-slate-700 px-3">
    <div class="min-w-0 flex-1 truncate text-sm font-semibold text-slate-900 dark:text-slate-100">예약된 알림/작업</div>
  ...
  <div class="flex shrink-0 gap-1 border-b border-slate-200 dark:border-slate-700 px-3 py-2">
    {#each filters as f}
      <button
        class={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${filter === f.value ? "bg-blue-600 text-white" : "border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700"}`}
  ...
  <div class="min-h-0 flex-1 overflow-y-auto px-3 py-3">
    ...
    {#if tasks.length === 0}
      <div class="px-1 py-3 text-sm text-slate-500 dark:text-slate-400">
        ...
        ＋로 새 예약을 만들거나, 텔레그램에서 <code class="rounded bg-slate-100 dark:bg-slate-800 px-1">!task</code> 또는 AI에게 요청해 등록하세요.
      </div>
    {/if}
    <div class="space-y-2">
        ...
        <div class="rounded-lg border border-slate-200 dark:border-slate-700 bg-white/85 dark:bg-slate-900/85 p-3 shadow-sm">
          <div class="flex items-start gap-2">
            <button class="min-w-0 flex-1 text-left" onclick={() => openEditor(t)} title="편집">
              <div class="truncate text-sm font-semibold text-slate-900 dark:text-slate-100" title={t.label}>{t.label || t.prompt}</div>
              <div class="mt-1 whitespace-pre-wrap break-words text-xs leading-5 text-slate-500 dark:text-slate-400">{t.prompt}</div>
            </button>
          ...
          <div class="mt-2 flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-slate-500 dark:text-slate-400">
            ...
            {#if t.status === "pending" && nextText(t)}
              <span class="font-semibold text-blue-600 dark:text-blue-400">→ {nextText(t)}</span>
            {/if}
            {#if t.script}
          ...
          <div class="mt-2 flex justify-end gap-1.5">
            <button class="rounded border border-slate-300 dark:border-slate-600 px-2 py-1 text-xs text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700" onclick={() => openEditor(t)}>✏️ 편집</button>
            {#if t.status === "pending"}
              <button class="rounded border border-slate-300 dark:border-slate-600 px-2 py-1 text-xs text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700" onclick={() => pause(t)}>⏸ 일시정지</button>
            {:else if t.status === "paused"}
              <button class="rounded border border-emerald-300 dark:border-emerald-700 px-2 py-1 text-xs text-emerald-700 dark:text-emerald-300 hover:bg-emerald-50" onclick={() => resume(t)}>▶ 재개</button>
            {/if}
            {#if t.status !== "cancelled"}
              <button class="rounded border border-rose-200 dark:border-rose-800 px-2 py-1 text-xs text-rose-700 dark:text-rose-300 hover:bg-rose-50 dark:hover:bg-rose-950/40" onclick={() => cancel(t)}>✕ 취소</button>
            {/if}
            <button class="rounded border border-rose-300 dark:border-rose-700 px-2 py-1 text-xs font-semibold text-rose-700 dark:text-rose-300 hover:bg-rose-50 dark:hover:bg-rose-950/40" onclick={() => del(t)}>🗑 삭제</button>
          </div>
        </div>
      {/each}
    </div>
...
{#if editing}
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4" onclick={(e) => { if (e.target === e.currentTarget) editing = null; }}>
    <div class="flex max-h-[88vh] w-[520px] max-w-full flex-col overflow-hidden rounded-lg bg-white dark:bg-slate-900 shadow-xl">
      <div class="border-b border-slate-200 dark:border-slate-700 px-4 py-3 text-sm font-semibold text-slate-900 dark:text-slate-100">{editing.id ? "예약 편집" : "새 예약"}</div>
      <div class="flex-1 space-y-3 overflow-y-auto px-4 py-3 text-left">
        <label class="block">
          <span class="mb-1 block text-xs font-medium text-slate-600 dark:text-slate-400">이름 (선택)</span>
          <input class="w-full rounded border border-slate-300 dark:border-slate-600 px-2 py-1.5 text-sm" bind:value={editing.label} placeholder="예: 구글 플레이 콘솔 재개 알림" />
        </label>
        <label class="block">
          <span class="mb-1 block text-xs font-medium text-slate-600 dark:text-slate-400">종류</span>
          <div class="flex gap-2">
            <button type="button" class={`flex-1 rounded border px-2 py-1.5 text-sm ${!editing.isTask ? "border-blue-500 dark:border-blue-400 bg-blue-50 dark:bg-blue-950/40 font-semibold text-blue-700 dark:text-blue-300" : "border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-400"}`} onclick={() => (editing.isTask = false)}>🔔 알림 (메시지만 전송)</button>
            <button type="button" class={`flex-1 rounded border px-2 py-1.5 text-sm ${editing.isTask ? "border-blue-500 dark:border-blue-400 bg-blue-50 dark:bg-blue-950/40 font-semibold text-blue-700 dark:text-blue-300" : "border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-400"}`} onclick={() => (editing.isTask = true)}>🤖 작업 (AI에게 실행 요청)</button>
          </div>
        </label>
        <label class="block">
          <span class="mb-1 block text-xs font-medium text-slate-600 dark:text-slate-400">{editing.isTask ? "AI에게 시킬 내용" : "알림 메시지"}</span>
          <textarea class="w-full resize-y rounded border border-slate-300 dark:border-slate-600 px-2 py-1.5 text-sm" rows="3" bind:value={editing.prompt} placeholder={editing.isTask ? "예: 빌드 로그를 확인하고 실패 시 요약해서 알려줘" : "예: 회의 시작 10분 전입니다"}></textarea>
        </label>
        <div class="block">
          <span class="mb-1 block text-xs font-medium text-slate-600 dark:text-slate-400">일정</span>
          <div class="mb-2 flex gap-2">
            <button type="button" class={`flex-1 rounded border px-2 py-1.5 text-sm ${editing.scheduleType === "once" ? "border-blue-500 dark:border-blue-400 bg-blue-50 dark:bg-blue-950/40 font-semibold text-blue-700 dark:text-blue-300" : "border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-400"}`} onclick={() => (editing.scheduleType = "once")}>일회성</button>
            <button type="button" class={`flex-1 rounded border px-2 py-1.5 text-sm ${editing.scheduleType === "cron" ? "border-blue-500 dark:border-blue-400 bg-blue-50 dark:bg-blue-950/40 font-semibold text-blue-700 dark:text-blue-300" : "border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-400"}`} onclick={() => (editing.scheduleType = "cron")}>반복</button>
          </div>
          {#if editing.scheduleType === "once"}
            <input type="datetime-local" class="w-full rounded border border-slate-300 dark:border-slate-600 px-2 py-1.5 text-sm" bind:value={editing.fireAtLocal} />
          {:else}
            <input class="w-full rounded border border-slate-300 dark:border-slate-600 px-2 py-1.5 font-mono text-sm" bind:value={editing.cronExpr} placeholder="0 9 * * *" />
            <div class="mt-1.5 flex flex-wrap gap-1.5">
              {#each cronPresets as p}
                <button type="button" class="rounded-full border border-slate-300 dark:border-slate-600 px-2 py-0.5 text-[11px] text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700" onclick={() => (editing.cronExpr = p.expr)}>{p.label}</button>
              {/each}
            </div>
            <span class="mt-1 block text-[11px] text-slate-400 dark:text-slate-500">5필드 cron 표현식 (분 시 일 월 요일)</span>
          {/if}
        </div>
      </div>
      {#if editorError}
