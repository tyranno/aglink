...
Telegram msg
 ...
 → runner.Run(cwd=project.Path, prompt=메시지, resume=conv.SessionID)
      → stream-json 이벤트 → relay가 Telegram 전송
      → 종료 시 session_id/result 확보
...
### 3.3 Worker 실행 입출력 / stream-json 이벤트

```go
type RunRequest struct {
    Prompt    string
    WorkDir   string // project.Path
    SessionID string // "" = 새 세션, 값 있으면 --resume
    Model     string // WorkerModel
}
type RunResult struct {
    SessionID string // 새 세션이면 캡처된 id
    Text      string // 최종 결과 텍스트
    IsError   bool
}

// claude --output-format stream-json 각 라인(JSON) 중 사용하는 필드만 방어적 파싱
type StreamEvent struct {
    Type      string `json:"type"`       // "system"|"assistant"|"result"|...
    Subtype   string `json:"subtype"`    // "init" 등
    SessionID string `json:"session_id"` // system/init 등에서 등장
    // assistant 메시지 텍스트, result 텍스트는 중첩 구조 → 헬퍼로 추출
}
```
...
claude -p "<routing system prompt + 대화목록 + 사용자메시지>" \
       --model <ManagerModel> --output-format json
```
- 결과 envelope의 `.result` 텍스트에서 **첫 `{...}` JSON 블록**을 추출해 `RouteDecision`으로 파싱.
- 파싱 실패/빈 결과 → 폴백(§6.3).
...
```
claude -p "<user message>" --output-format stream-json \
       --dangerously-skip-permissions \
       [--model <WorkerModel>] [--resume <SessionID>]
   (cwd = project.Path)
```
- stream-json 라인 스트리밍 → `onEvent`. `system/init`에서 `session_id` 캡처(새 대화 저장), `assistant`/`result`에서 텍스트 수집.
- `--resume <SessionID>`로 대화 연속. 새 대화는 미지정 후 캡처.
...
| stream-json 샘플 | init/assistant/result 라인 모음 |
...
teleclaude/
  ...
  bot.go         # 폴링, auth, 명령 디스패치, FIFO 큐, /cancel
  manager.go     # ClaudeClient.Route 오케스트레이션, clarify, 요약 갱신
  store.go       # StoreRepo(JSON) 구현, 프로젝트/대화 CRUD, 영속
  runner.go      # ClaudeClient 구현: Route/Run claude 호출, stream-json, cancel
