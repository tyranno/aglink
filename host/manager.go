...
// expired and could not be refreshed. Unlike isSessionNotFound, the CLI exits 0
// here and puts the message in the result body ("Failed to authenticate. API
// Error: 401 OAuth access token has expired. Re-authenticate to continue."), so
// without this check the worker records the error string as the assistant's
// answer, relays it to the user, and — worse — marks the conversation Started
// while no CLI session was ever created. Every later turn then resumes a session
// id that does not exist and dies with "No conversation found with session ID".
