...
```
[폰 Telegram] ──(long polling)──> [aglink.exe (Go, 단일 바이너리)]
...
.\aglink.exe run     # 설정 없으면 마법사 자동 시작 (또는 .\aglink.exe setup)
```
...
> claude CLI는 OAuth/keychain 로그인을 사용합니다. aglink는 그 로그인을 그대로 위임합니다(별도 API 키 불필요).
...
## 4. 설정 파일 `%USERPROFILE%\.aglink\config.txt`
...
### 방법 A — 봇에서 명령 (권장)
```
/project add aglink   C:\Project\88.MyProject\aglink
/project add voicesvr C:\Project\88.MyProject\voice-chat-server
/project list
```

### 방법 B — 파일 직접 작성 `%USERPROFILE%\.aglink\store.json`
```json
{
  "active": { "project": "", "conversationId": "" },
  "projects": {
    "aglink":   { "path": "C:\\Project\\88.MyProject\\aglink", "conversations": {} },
...
```powershell
cd C:\Project\88.MyProject\aglink
go build -o aglink.exe .
.\launcher.ps1                             # aglink.exe 실행 + 자동 재시작 루프
```

`launcher.ps1`은 aglink.exe를 감싸는 루프입니다. `!update` 명령으로 텔레그램에서 새 버전을 빌드하면 자동으로 교체·재시작합니다(아래 §6-1).

정상 기동 로그:
```
[launcher] aglink 시작 (C:\Project\88.MyProject\aglink)
...
### 직접 실행 (업데이트 불필요한 경우)
```powershell
.\aglink.exe run                       # 기본 config.txt 사용
.\aglink.exe run C:\path\to\config.txt # 다른 설정 파일
```
...
# 중지:
Get-Process aglink | Stop-Process -Force
```
...
[텔레그램] !update
    ↓
aglink: go build -o aglink_new.exe . 실행 (소스 디렉토리)
    ↓ 빌드 성공
"✅ 빌드 성공! launcher.ps1이 교체 후 재시작합니다." 전송
    ↓
aglink exit(42) → 봇 일시 오프라인 (~5~10초)
    ↓
launcher.ps1: exit 42 감지 → aglink_new.exe → aglink.exe 교체 → 재시작
    ↓
...
```
나: aglink 프로젝트가 뭐 하는 건지 한 줄로 설명해줘
봇: 📂 aglink · 💬 aglink 설명 (새 대화)
    <claude가 그 폴더를 읽고 답변>

나: 방금 그거 어느 언어로 짰어?
봇: 📂 aglink · 💬 aglink 설명 (이어가기)   ← 같은 대화 resume
    Go 입니다.
...
| `!chat list` | 대화 목록 |
| `!chat use <id>` / `!chat use <project> <id>` | 대화 수동 전환 |
...
- `config.txt`, `store.json`은 `%USERPROFILE%\.aglink\`에 있으며 git에 올라가지 않습니다(.gitignore).
...
| 설정/코드 바꾼 게 반영 안 됨 | launcher.ps1 사용 중이면 텔레그램 `!update`로 자동 빌드·재시작. 직접 실행 중이면 `Stop-Process aglink` → `go build` → 다시 `run` |
