...
// logFileName is the diagnostic log under the data dir (~/.aglink).
const logFileName = "aglink.log"

// maxLogBytes caps the log at startup. Past this the current file is rotated to
// aglink.log.old (one generation) so the log can't grow without bound.
...
// captured — not just aglink's "restarting" line.
var childLogWriter io.Writer = os.Stderr

// setupFileLogging tees the standard logger to <dir>/aglink.log in addition
// to stderr, and returns a close func.
//
// This exists because the elevated instance is launched with SW_HIDE (see
// relaunchElevated), so its console — and every log line aglink and its
