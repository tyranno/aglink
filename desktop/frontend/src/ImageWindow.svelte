<script>
  ...
  let zoom = $state(1);
  let drag = null; // { mode: "move" | "resize", sx, sy, ox, oy, ow, oh }
  let scrollEl;
  ...
  function onWheel(e) {
    if (!e.ctrlKey) return; // plain wheel = normal pan-scroll
    e.preventDefault();
    zoom = clampZoom(zoom * (e.deltaY < 0 ? 1.15 : 1 / 1.15));
  }

  // Wheel must be a non-passive listener so Ctrl+wheel can preventDefault (block
  // the app's own zoom/scroll). Re-runs when the image area mounts/unmounts.
  $effect(() => {
    const el = scrollEl;
    if (!el) return;
  ...
  // Image bounds in px, derived from the window size so zoom=1 fits the window
  // and zoom>1 overflows into a pan-scrollable area (deterministic, no %-height quirks).
...
{#if chat.lightboxSrc}
  ...
  >
    ...
    <div
      class="flex h-8 shrink-0 cursor-move select-none items-center gap-1 bg-slate-800 px-2 text-white"
      onmousedown={(e) => startDrag("move", e)}
      role="toolbar"
      tabindex="-1"
    >
      <span class="mr-auto truncate text-xs font-medium">이미지 보기 · 드래그 이동 / Ctrl+휠 확대</span>
      <button class="grid h-6 w-6 place-items-center rounded text-base leading-none hover:bg-white/20" onmousedown={(e) => e.stopPropagation()} onclick={zoomOut} title="축소 (Ctrl+휠↓)" aria-label="축소">−</button>
      <button class="grid h-6 min-w-[3.2rem] place-items-center rounded px-1 text-[11px] tabular-nums hover:bg-white/20" onmousedown={(e) => e.stopPropagation()} onclick={zoomReset} title="원래 크기(맞춤)로" aria-label="원래대로">{Math.round(zoom * 100)}%</button>
      <button class="grid h-6 w-6 place-items-center rounded text-base leading-none hover:bg-white/20" onmousedown={(e) => e.stopPropagation()} onclick={zoomIn} title="확대 (Ctrl+휠↑)" aria-label="확대">＋</button>
      <button class="ml-1 grid h-6 w-6 place-items-center rounded text-sm hover:bg-white/20" onmousedown={(e) => e.stopPropagation()} onclick={closeLightbox} title="닫기 (Esc)" aria-label="닫기">✕</button>
    </div>

    <!-- image area: fits at 100%, pan-scrolls when zoomed in -->
    <div bind:this={scrollEl} class="min-h-0 flex-1 overflow-auto bg-slate-100">
